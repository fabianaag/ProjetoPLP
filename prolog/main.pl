...
pulo(Opcao, Acertos, Resposta, Pulos) :- (Pulos =< 3 -> (Resposta =:= "p" -> Pulos1 is Pulos + 1,writeln("Pulou"), loop(Opcao, Acertos, Pulos1); true); writeln("não pode mais pular"), leitura_resposta(Resposta)).


loop(Opcao, Acertos, Pulos):-
      ...
      leitura_resposta(Resposta),
        ...
        pulo(Opcao, Acertos, Resposta, Pulos),
